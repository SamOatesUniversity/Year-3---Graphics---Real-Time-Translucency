...
void
Window::setView(std::shared_ptr<WindowViewDelegate> view)
{
	if (view == view_) {
		return;
    }
...
Window::controller() const
{
    return controller_;
}

void
Window::setController(std::shared_ptr<WindowControlDelegate> controller)
{
...
void
Window::setTitle(std::string newTitle)
{
    glfwSetWindowTitle(newTitle.c_str());
}

void
Window::resize(int width,
               int height)
{
...
void
Window::onMouseButton(int button,
                      int action)
{
    // NB: GLFW limitation limits operation to a single window
    Window* window = main_window_.get();
